...
export function PropertiesPanel() {
  ...
  return (
    <div className="fixed top-16 md:top-6 right-4 md:right-6 z-40 w-[calc(100vw-2rem)] md:w-64 max-h-[80vh] overflow-y-auto bg-zinc-900/95 dark:bg-zinc-950/95 backdrop-blur-xl border border-white/10 dark:border-white/5 rounded-2xl shadow-xl flex flex-col text-xs [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
      <div className={`flex justify-between items-center p-3 ${isCollapsed ? '' : 'border-b border-white/10'}`}>
        <button
          onClick={() => setIsCollapsed(!isCollapsed)}
          className="flex items-center gap-2 font-semibold text-zinc-200 hover:text-white transition-colors cursor-pointer"
        >
          <svg
